...
func registerSession(srv *mcp.Server, sess *browser.Session) {
	...
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "session",
		Description: "Recover or switch identity. mode=reset relaunches the browser when wedged (op-timeout, dead session, unresponsive SPA). mode=clear wipes cookies+storage+reload for a clean slate. mode=profile manages named browser profiles (isolated cookies/auth/storage): action=list|create|switch|delete|current|export|import. reset/clear return fresh orientation; profile actions return status.",
