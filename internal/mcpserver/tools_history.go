...
func registerHistory(srv *mcp.Server, sess *browser.Session) {
	type args struct {
		...
		Errors bool `json:"errors,omitempty" jsonschema:"if true, show only entries where the action was blocked (a CHALLENGE verdict)"`
	}
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "history",
		Description: "Recall the session's action log - what you've done on the page, with each action's verdict + URL - offloaded from your context so a long task doesn't bloat it. Each entry: step number, time, action, verdict, url. Pass last=N for the recent N; errors=true for blocked (CHALLENGE) actions only. Use it to re-orient after a long flow ('where am I, what did I just do, did that last click work') instead of re-snapshotting or relying on your own context. Step numbers are monotonic across the whole session.",
