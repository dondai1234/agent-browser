...
func registerAct(srv *mcp.Server, sess *browser.Session) {
	...
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "act",
		Description: "Interact with the page. One call returns a verdict + delta - do NOT re-see after (the verdict tells you what happened). Name a control (intent) or give ref/selector. Modes: click (default for buttons/links), fill (value= for inputs), select (value= for dropdowns), hover (hover=true), key (key= for keypress), upload (files=[..]), batch form fill (fields={label:value}). Verdicts are self-diagnosing: includes 'did you mean?' hints for not-found elements, visible error messages, and actionable suggestions when the action didn't clearly succeed. Confidence tag: [confirmed] / [likely] / [uncertain].",
