# OpenCode Live Comparison: agent-browser vs @playwright/mcp
...
|--------|---------|--------|
| agent-browser | `agent-browser mcp` (stdio) | Connected |
...
**Winner:** agent-browser (fewer calls, less I/O)
...
|------|-------|--------|-------|
| agent-browser | 2 | Extracted all stories as flat text array | `extract` with `.titleline, .score` worked clean |
| @playwright/mcp | 2 | Extracted structured JSON with title/score/link | `evaluate` gave me control over output shape |

**Takeaway:** agent-browser's `extract` is faster to write but gives flat text. Playwright's `evaluate` takes more code but returns structured data I can use directly.

### Test 2: GitHub trending (Stack Overflow was Cloudflare-blocked for both)

| Tool | Calls | Result | Notes |
|------|-------|--------|-------|
| agent-browser | 2 | Got all 17 repos with full text | `extract` pulled everything in one selector |
| @playwright/mcp | 2 | Got top 5 repos as structured JSON | Had to write JS to parse the DOM |

**Takeaway:** For bulk text extraction, agent-browser wins. For structured data, playwright wins.

### Test 3: Multi-tab workflow

| Tool | Calls | Result | Notes |
|------|-------|--------|-------|
| agent-browser | 3 | Opened 2 tabs, listed, switched | `tabs` action is clean, shows URLs and titles |
| @playwright/mcp | 2 | Opened 1 tab, listed, switched | `tabs` shows index-based list, auto-currents new tab |

**Takeagent-browser:** Both handle tabs well. agent-browser's `tabs list` output is slightly more readable.

### Test 4: Error recovery

| Scenario | agent-browser | @playwright/mcp |
|----------|---------------|-----------------|
...
**Takeaway:** agent-browser gives more helpful error messages with recovery suggestions. Playwright's errors are terser.

---

## 7. Personal Preference (After Extended Use)

**What I reach for first:** `agent-browser`
...
**Honest assessment:** agent-browser feels like a well-designed CLI tool. Playwright feels like a raw API. agent-browser makes the common case easy; playwright makes the complex case possible. For 80% of browser automation tasks, agent-browser is the better DX. For the remaining 20% (complex JS extraction, batch operations), playwright is more powerful.

**Final verdict: I prefer agent-browser for daily use, but I'm glad playwright exists for the edge cases.**
