...
func runMCP(args []string) {
	...
	sess, err := browser.New(browser.Config{Headless: *headless, UserDataDir: userDataDir, Proxy: *proxy, UserAgent: *userAgent, ViewportW: w, ViewportH: h, Stealth: !*noStealth, OpTimeout: *opTimeout, IdleTimeout: *idleTimeout}) // 0 = long-lived session
	if err != nil {
	...
	opts := &mcp.ServerOptions{
		Instructions: "agent-browser v2: token-efficient browser automation built for the agent. ORIENT: navigate/see level=brief -> a page brief (type, auth, primary actions with refs, regions); level=summary -> the ref list (interactive elements; decorative role=button divs/ad slots with no name + no focus are dropped, real icon-only buttons + unlabeled inputs stay). ACT BY INTENT: act \"Sign in\" or act \"Username\" value=x resolves a control by name and clicks/fills it in one call (local heuristics, no LLM). Two-tier match: the a11y label first, then the DOM name/id/placeholder/title/aria-label as a fallback for poorly-labeled inputs. Several matches -> it returns ranked candidates (disambiguate with nth or role; nth=-1 = last of N identical buttons, -2 = second-last, so you don't have to count). Every action returns a VERDICT (navigated to / dialog opened / status / changed / page updated / no visible effect / CHALLENGE) + a DELTA (what changed, fresh refs) so you rarely re-see; a URL-stable reorder (sort/filter/SPA re-render) now reports 'page updated' instead of the misleading 'no visible effect'. non-nav actions also fold in the XHRs that fired (net:). If an action fires but the page can't re-snapshot (navigating/wedged) you get a soft 'action fired; call see to refresh refs' verdict, not an error. EXTRACT: extract table/links/list/form/article -> JSON. CSS-SELECTOR ESCAPE HATCH: find/click/fill/act selector=\"css\" (e.g. \"div[role=widget]\", \".btn-x\") reaches elements the a11y tree does NOT surface (custom widgets, presentational nodes) - the one place a pure a11y-tree tool would otherwise miss; find selector returns [css] lines with a sel= you pass back to click/fill/act. RECALL: history (the session action log, offloaded from your context); history errors=true shows blocked (CHALLENGE) or failed (error) actions. WAIT: wait url=/text=/gone= (seconds=0 with a condition defaults to 10s). FILL: fill fields={ref:value} for a whole form. By-ref tools (click/fill/select/hover/press_key) use STABLE refs: the same element keeps its ref after a re-render (no silent retargeting when tree order shifts); a ref only errors 'call see to refresh' if the element is actually gone. press_key takes ONE named key or ONE char; pass ref to focus a field first (press_key Enter ref=r3 submits that form). Refs are per-tab, cleared on navigation (fresh page = fresh low refs); reset zeroes them. QoL: navigate action=back/forward/reload; scroll ref=r12 to bring an off-screen element into view; read on a link ref also returns its href; screenshot fullPage=true or ref=r12; where for a ~30-token re-orientation (url, page, auth, last action, tab, scroll). RECOVER: reset re-navigates the current tab to a fresh page and KEEPS your other tabs when the browser is alive; it relaunches Chrome (other tabs lost) only when the browser is dead. Every browser op is bounded by an op timeout (default 30s) so a hung page returns an error instead of wedging the session. eval unquotes string results (document.title -> Title, not \"Title\") and covers the rest.",
	}
