...
func runMCP(args []string) {
	...
	opts := &mcp.ServerOptions{
		Instructions: "goshawk v4: 9 browser automation tools (nav, see, act, js, find, tabs, history, session, login). refs from see/find are stable across re-renders (auto-healed) and per-tab (cleared on nav). Trust verdicts - don't re-see after act. Op timeout 20s.",
	}
