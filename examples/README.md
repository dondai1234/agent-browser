...
> If your client reports `spawn goshawk ENOENT`, it can't find the binary on
> its PATH. Use the absolute path in `command`: `$(go env GOPATH)/bin/goshawk`
...
| Claude Code | `mcpServers` | `~/.claude/settings.json` (user) or `.mcp.json` (project, team-shareable) | `claude mcp add goshawk -- goshawk mcp` |
...
```sh
openclaw mcp add goshawk --command goshawk --arg mcp
openclaw mcp doctor --probe goshawk
```

(OpenClaw is deliberately token-stingy about MCP tool defs (same philosophy as
goshawk), so it's a natural fit.)
...
the tool-def token tax, the exact problem goshawk exists to solve). So
...
`goshawk` extension already exists there as a reference. A pi extension
...
- **Persistence**: by default goshawk keeps a Chrome profile at
  `<os config dir>/goshawk` so logins/cookies survive restarts - the agent
